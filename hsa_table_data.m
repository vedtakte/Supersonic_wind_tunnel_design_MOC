...
M1_vec  = zeros(num_rows, 1); % Mach Number at state 1

Nu2_vec = zeros(num_rows, 1);
Mu2_vec = zeros(num_rows, 1);
M2_vec  = zeros(num_rows, 1); % Mach Number at state 2

Lambda_vec = zeros(num_rows, 1);

for i = 1:length(nu_vals)-1
    % State 1 (Start of wave)
    Nu1_vec(i) = nu_vals(i);
    Mu1_vec(i) = mu_vals(i);
    M1_vec(i)  = M_vals(i);
...
CharacteristicTable = table(Nu1_vec, Mu1_vec, M1_vec, Nu2_vec, Mu2_vec, M2_vec, Lambda_vec, ...
    'VariableNames', {'nu1_deg', 'mu1_deg', 'Mach1', 'nu2_deg', 'mu2_deg', 'Mach2', 'Lambda_char_deg'});

% Display the table in Command Window
disp('Characteristic Angles Table (with Mach Numbers):');
